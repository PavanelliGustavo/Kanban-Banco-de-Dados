...
class KanbanViewFrame(tk.Frame):
    ...
    def setUpKanbanBoard(self):
        ...
        # --- DADOS ---
        columns_data = [c for c in mock_db.COLUMNS_DB if c['obra_id'] == self.current_work_id]
        if not columns_data:
             columns_data = [
                 {"titulo": "Em Planejamento", "posicao_visual": 1},
                 {"titulo": "Concluído", "posicao_visual": 99}
             ]
        columns_data.sort(key=lambda x: int(x.get('posicao_visual', 999)))
        ...
        for idx, col_data in enumerate(columns_data):
            ...
            col_frame = tk.Frame(scrollable_frame, bg="#e0e0e0", bd=2, relief="groove", width=260)
            ...
            header_lbl = tk.Label(col_frame, text=header_text, font=("Helvetica", 11, "bold"), bg="#e0e0e0", fg="#555", pady=10)
    ...
    def createCard(self, parent, task):
        ...
        lbl = tk.Label(card, text=display_text, font=("Helvetica", 10, "bold"), bg="white", wraplength=230, justify="left")
        lbl.pack(anchor="w")
    ...
    def open_edit_form(self, task_data, is_new):
        ...
        entries = {}
        available_cols_objs = [c for c in mock_db.COLUMNS_DB if c['obra_id'] == self.current_work_id]
        available_cols_objs.sort(key=lambda x: x.get('posicao_visual', 99))
        col_names = [c['titulo'] for c in available_cols_objs]
        ...
        val_s = col_names[0] if is_new and col_names else task_data['status']
        ...
        add_field("Colunas", "status", val_s, "combo", col_names)
    ...
    # -------------------------------------------------------------------------
    # GESTÃO DE COLUNAS (CRIAR E EDITAR)
    # -------------------------------------------------------------------------

    def open_column_modal(self, col_data, is_new):
        modal = tk.Toplevel(self)
        ...
        tk.Label(modal, text=title_text, font=("Helvetica", 14, "bold"), bg="white", fg="#9C27B0").pack(pady=20)

        entries = {}

        val_title = "" if is_new else col_data['titulo']
        val_pos = "" if is_new else str(col_data['posicao_visual'])

        # Campo Título
        tk.Label(modal, text="Título da Coluna:", bg="white", anchor="w").pack(fill="x", padx=30, pady=(5,0))
        ...
        # Campo Posição
        tk.Label(modal, text="Posição (Esq. para Dir.):", bg="white", anchor="w").pack(fill="x", padx=30, pady=(15,0))
    ...
    def check_column_errors(self, parent_modal, entries, col_data, is_new):
        title = entries['titulo'].get().strip()
        pos = entries['posicao'].get().strip()

        if not title:
            messagebox.showwarning("Atenção", "O título da coluna não pode ser vazio.", parent=parent_modal)
            return
        
        if not pos:
            messagebox.showwarning("Atenção", "A posição da coluna não pode ser vazia.", parent=parent_modal)
            return
        
        # Validação de Número Natural (Inteiro >= 1)
        if not pos.isdigit() or int(pos) < 1:
            messagebox.showwarning("Atenção", "A posição deve ser um número inteiro positivo (Ex: 1, 2, 3).", parent=parent_modal)
            return

        self.confirm_column_save_popup(parent_modal, title, int(pos), col_data, is_new)

    def confirm_column_save_popup(self, parent_modal, title, pos, col_data, is_new):
        ...
        tk.Label(popup, text="Deseja salvar as mudanças?", font=("Helvetica", 11), bg="white").pack(pady=30)
        ...
        def save_action():
            if is_new:
                new_id = 900
                if mock_db.COLUMNS_DB:
                    new_id = max(c['id'] for c in mock_db.COLUMNS_DB) + 1
                ...
                mock_db.COLUMNS_DB.append(new_col)
            else:
                # Atualizar nome da coluna
                old_title = col_data['titulo']
                col_data['titulo'] = title
                col_data['posicao_visual'] = pos
                
                # ATUALIZAR TASKS: Se o nome da coluna mudou, movemos os cards para o novo nome
                if old_title != title:
                    for task in mock_db.KANBAN_TASKS_DB:
                        if task['obra_id'] == self.current_work_id and task['status'] == old_title:
                            task['status'] = title
        ...
        tk.Button(btns, text="SIM", bg="#4CAF50", fg="white", width=10, command=save_action).pack(side="left", padx=10)
        tk.Button(btns, text="NÃO", bg="#f44336", fg="white", width=10, command=popup.destroy).pack(side="right", padx=10)

    def confirm_column_delete_popup(self, parent_modal, col_data):
        popup = tk.Toplevel(self)
        ...
        def delete_action():
            # Excluir Coluna
            mock_db.COLUMNS_DB.remove(col_data)
            
            # Excluir Cards vinculados a esta coluna (status = titulo)
            # Removemos da lista global filtrando quem NÃO é dessa coluna
            col_title = col_data['titulo']
            # Atenção: filter cria um iterador, precisamos converter para lista
            # Mas para modificar a lista original in-place ou reatribuir no mock_db:
            
            # Estratégia de remoção segura:
            to_remove = [t for t in mock_db.KANBAN_TASKS_DB 
                         if t['obra_id'] == self.current_work_id and t['status'] == col_title]
            
            for item in to_remove:
                mock_db.KANBAN_TASKS_DB.remove(item)
        ...
        tk.Button(btns, text="SIM", bg="#4CAF50", fg="white", width=10, command=delete_action).pack(side="left", padx=10)
        tk.Button(btns, text="NÃO", bg="#f44336", fg="white", width=10, command=popup.destroy).pack(side="right", padx=10)

    # -------------------------------------------------------------------------
    # CONFIRMAÇÃO DE CARDS
    # -------------------------------------------------------------------------
        ...
        def yes():
            if action == "delete":
                ...
                if not t or not s or not p or not d or not pos_str:
                    popup.destroy()
                    messagebox.showwarning("Erro", "Todos os campos são obrigatórios.", parent=parent_modal)
                    return
                
                if not re.match(r"^\d{2}/\d{2}/\d{4}$", p):
                    popup.destroy()
                    messagebox.showwarning("Erro", "O Prazo deve estar no formato NN/NN/NNNN.", parent=parent_modal)
                    return
