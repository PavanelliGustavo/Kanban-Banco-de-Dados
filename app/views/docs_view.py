...
class DocsViewFrame(tk.Frame):
    ...
    TITLE_PARAMS = {"font": (TEXT_FONT, 24, "bold"), "bg": HEADER_FRAME_BG, "fg": "#333"}
    SUBTITLE_PARAMS = {"font": (TEXT_FONT, 10, "italic"), "bg": HEADER_FRAME_BG, "fg": "#666"}
    # endregion
    ...
    def setUpHeader(self):
        ...
        def back_command():
            self.controller.show_kanban_frame(self.current_work_id, self.current_work_name)

        tk.Button(header_frame, text=self.BACK_BUTTON_TEXT, command=back_command,
                  **self.BACK_BUTTON_PARAMS).pack(side="left")
        
        tk.Label(header_frame, text=self.TITLE_TEXT, **self.TITLE_PARAMS).pack(side="left", padx=30)
        
        subtitle_text = f"Referente a: {self.current_work_name}"
        tk.Label(header_frame, text=subtitle_text, **self.SUBTITLE_PARAMS).pack(side="bottom", anchor="w")
    ...
    def setUpFilterArea(self):
        ...
        # 1. Ordenação
        tk.Label(left_area, text=self.FILTER_LABEL_TEXT, bg=self.FILTER_FRAME_BG).pack(side="left")
        
        self.combo_sort = ttk.Combobox(left_area, values=self.COMBOBOX_VALUES, width=20, state="readonly")
        ...
        tk.Label(left_area, text="|", bg=self.FILTER_FRAME_BG, fg="#888").pack(side="left", padx=5)
        
        tk.Label(left_area, text="Pesquisar:", bg=self.FILTER_FRAME_BG).pack(side="left", padx=5)
    ...
    def apply_sort_and_search(self):
        # 1. Cópia dos dados
        data = self.current_docs[:]
        
        # 2. Pesquisa
        if hasattr(self, 'entry_search'):
            term = self.entry_search.get().lower().strip()
            if term:
                data = [d for d in data if term in d['titulo'].lower()]

        # 3. Ordenação
        sort_opt = self.combo_sort.get()
        if sort_opt == "A-Z (Nome)":
            data.sort(key=lambda x: x['titulo'])
        elif sort_opt == "Z-A (Nome)":
            data.sort(key=lambda x: x['titulo'], reverse=True)
        elif sort_opt == "Mais Recentes":
            data.sort(key=lambda x: x['data'], reverse=True)
    ...
    def on_open_document(self, event=None):
        ...
        doc_data = next((d for d in mock_db.DOCUMENTS_DB if d['id'] == doc_id), None)
    ...
    def simulate_open_file(self, doc_data):
        messagebox.showinfo("Abrir Arquivo", f"O arquivo '{doc_data['titulo']}' seria aberto agora.")

    # -------------------------------------------------------------------------
    # OPÇÕES E MODAIS (EMPRESA)
    # -------------------------------------------------------------------------
    ...
    def open_file_options_modal(self, doc_data):
        ...
        tk.Label(options_window, text="O que deseja fazer?", font=("Helvetica", 12, "bold"), bg="white").pack(pady=20)
    ...
    def confirm_delete_popup(self, parent_modal, doc_data):
        ...
        tk.Label(popup, text="Deseja excluir o documento?", font=("Helvetica", 12), bg="white").pack(pady=25)
    ...
    def perform_delete(self, doc_data, popup, parent_modal):
        mock_db.DOCUMENTS_DB.remove(doc_data)
        popup.destroy()
    ...
    def open_document_modal(self, is_new, doc_data=None):
        ...
        tk.Label(modal, text=title, font=("Helvetica", 14, "bold"), bg="white", fg="#2196F3").pack(pady=20)

        # Nome
        tk.Label(modal, text="Nome de Exibição:", font=("bold", 10), bg="white", anchor="w").pack(fill="x", padx=30, pady=(5, 0))
        entry_name = tk.Entry(modal, bg="#fafafa")
        entry_name.pack(fill="x", padx=30, pady=(0, 10))

        # Arquivo
        tk.Label(modal, text="Arquivo PDF:", font=("bold", 10), bg="white", anchor="w").pack(fill="x", padx=30, pady=(5, 0))
        ...
        tk.Button(file_frame, text="...", command=select_file, width=3).pack(side="left", padx=(5,0))
    ...
    def confirm_save_popup(self, parent_modal, entry_name, entry_path, is_new, doc_data):
        ...
        if not name:
            messagebox.showwarning("Erro", "O arquivo inserido precisa ter um Nome", parent=parent_modal)
            return
        
        if not path:
            messagebox.showwarning("Erro", "Arquivo não foi adicionado", parent=parent_modal)
            return

        if not path.lower().endswith('.pdf'):
            messagebox.showwarning("Erro", "Arquivo precisa estar em formato PDF", parent=parent_modal)
            return
        ...
        tk.Label(popup, text="Deseja salvar as mudanças?", font=("Helvetica", 12), bg="white").pack(pady=25)
        btn_frame = tk.Frame(popup, bg="white")
        btn_frame.pack()
    ...
    def perform_save(self, is_new, doc_data, name, path, popup, parent_modal):
        today = datetime.date.today().strftime("%Y-%m-%d")

        if is_new:
            new_id = max([d['id'] for d in mock_db.DOCUMENTS_DB] or [0]) + 1
            new_doc = {"id": new_id, "obra_id": self.current_work_id, "titulo": name, "tipo": "PDF", "data": today, "caminho": path}
            mock_db.DOCUMENTS_DB.append(new_doc)
        else:
            doc_data['titulo'] = name
            doc_data['caminho'] = path
